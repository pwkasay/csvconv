class BeerConverter
  ...
  def self.convert(file:)
            ...
            if catalogue[(item_name.match /.*%/).to_s].blank?
              catalogue[(item_name.match /.*%/).to_s] = {
                ...
                totals: {
                  ...
                  x400ml: (BigDecimal.new(item_quantity) if item_name.include? "[12x400ml]").to_f,
                  ...
                  exportx400ml: (BigDecimal.new(item_quantity) if ((item_name.include? "[12x400ml]") && ((!uk.include? country) && (!country.blank?)))).to_f,
                  ...
                  ukx400ml: (BigDecimal.new(item_quantity) if ((item_name.include? "[12x400ml]") && (uk.include? country || country.blank?))).to_f,
                ...
                orders: [
                  {
                    ...
                    x400ml: (BigDecimal.new(item_quantity) if item_name.include? "[12x400ml]"),
              ...
              catalogue[(item_name.match /.*%/).to_s][:totals][:x400ml] += (BigDecimal.new(item_quantity) if item_name.include? "[12x400ml]").to_f
              ...
              catalogue[(item_name.match /.*%/).to_s][:totals][:exportx400ml] += (BigDecimal.new(item_quantity) if ((item_name.include? "[12x400ml]") && ((!uk.include? country) && (!country.blank?)))).to_f
              ...
              catalogue[(item_name.match /.*%/).to_s][:totals][:ukx400ml] += (BigDecimal.new(item_quantity) if ((item_name.include? "[12x400ml]") && ((uk.include? country) || (country.blank?)))).to_f
    ...
    headers = ["Product Name","% ABV","Quantity","","Export","24 x 330ml","12 x 750ml","12 x 500ml", "24 x 400ml", "30L Keg","20L Keg","","UK","24 x 330ml","12 x 750ml", "12 x 500ml", "24 x 400ml", "30L Keg","20L Keg","","Export HL","Export HL%","UK Duty HL","UK Duty HL%","LSBD","HSBD"]
    ...
    CSV.generate(write_headers: true, headers: headers) do |csv|
      catalogue.each do |beer, stats|
        csv << [
          ...
          stats[:totals][:ukx400ml],
          stats[:totals][:ukk30L],
          stats[:totals][:ukk20L],
          "",
          ((((stats[:totals][:exportx330ml].to_f)*0.0792)+((stats[:totals][:exportx750ml].to_f)*0.09)+((stats[:totals][:exportx500ml].to_f)*0.06)+((stats[:totals][:exportx400ml].to_f)*0.1056)+((stats[:totals][:exportk30L].to_f)*0.29)+((stats[:totals][:exportk20L].to_f)*0.19)).to_f).round(2),
          (((((stats[:totals][:exportx330ml].to_f)*0.0792)+((stats[:totals][:exportx750ml].to_f)*0.09)+((stats[:totals][:exportx500ml].to_f)*0.06)+((stats[:totals][:exportx400ml].to_f)*0.1056)+((stats[:totals][:exportk30L].to_f)*0.29)+((stats[:totals][:exportk20L].to_f)*0.19))*((stats[:info][:abv]).to_f)).to_f).round(2),
          ((((stats[:totals][:ukx330ml].to_f)*0.0792)+((stats[:totals][:ukx750ml].to_f)*0.09)+((stats[:totals][:ukx500ml].to_f)*0.06)+((stats[:totals][:ukx400ml].to_f)*0.1056)+((stats[:totals][:ukk30L].to_f)*0.29)+((stats[:totals][:ukk20L].to_f)*0.19)).to_f).round(2),
          ((((stats[:totals][:ukx330ml].to_f)*0.0792)+((stats[:totals][:ukx750ml].to_f)*0.09)+((stats[:totals][:ukx500ml].to_f)*0.06)+((stats[:totals][:ukx400ml].to_f)*0.1056)+((stats[:totals][:ukk30L].to_f)*0.29)+((stats[:totals][:ukk20L].to_f)*0.19))*((stats[:info][:abv]).to_f).to_f).round(2),
          ((((stats[:totals][:ukx330ml].to_f)*0.0792)+((stats[:totals][:ukx750ml].to_f)*0.09)+((stats[:totals][:ukx500ml].to_f)*0.06)+((stats[:totals][:ukx400ml].to_f)*0.1056)+((stats[:totals][:ukk30L].to_f)*0.29)+((stats[:totals][:ukk20L].to_f)*0.19))*((stats[:info][:abv]).to_f)*(9.54.to_f)).round(2),
          if stats[:info][:abv].to_f > 7.4
            (((((stats[:totals][:ukx330ml].to_f)*0.0792)+((stats[:totals][:ukx750ml].to_f)*0.09)+((stats[:totals][:ukx500ml].to_f)*0.06)+((stats[:totals][:ukx400ml].to_f)*0.1056)+((stats[:totals][:ukk30L].to_f)*0.29)+((stats[:totals][:ukk20L].to_f)*0.19))*((stats[:info][:abv]).to_f).to_f)*5.69.to_f).round(2)
          end
        ]
      end
    end
